...
import { SERVER_CONFIG, LANDING, TOOLS } from "./config.js";
...
const T = {
  "pt-BR": {
    ...
    ferramentas: (n: number) => `As ${n} ferramentas`,
    endpoints: "Endpoints",
    links: "Links",
    contato: "Contato",
  ...
  en: {
    ...
    comoUsarTexto: (rota: string, site: string) =>
      ...
      `Nothing to install.`,
    ferramentas: (n: number) => `The ${n} tools`,
    endpoints: "Endpoints",
    links: "Links",
    contato: "Contact",
...
<h2>${t.ferramentas(TOOLS.length)}</h2>
<ul>
  ${TOOLS.map((f) => `<li><code>${esc(f.name)}</code> — ${esc(f.title)}</li>`).join("\n  ")}
</ul>
